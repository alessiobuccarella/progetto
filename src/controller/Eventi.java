...
public class Eventi {
	public static void cliccato(Mano mano, int indiceCarta, JButton bottone, PostazioneGiocatore postazione, Piatto piatto, int turno, Mano manoOvest, Mano manoNord, Mano manoEst,PostazioneLaterale postazioneOvest, PostazioneGiocatore postazioneNord, PostazioneLaterale postazioneEst, JButton[] listaBottoni, Mazzo mazzo) {
		...
		for (int i=0;i<listaBottoni.length;i++) {if (listaBottoni[i]==null)System.out.println(1);}
		...
		
		{
			...
			if (Menu.cartaScarto.getV()==10) {Menu.turno=2; }
			...
			if (Menu.turno==1) 
			{
				 ...
				 {
					 postazioneOvest.add(new JLabel(new ImageIcon("./src/immagini/dorso90.png")));postazioneOvest.add(Box.createRigidArea(new Dimension(0,5)));}
				 ...
				 {
					 postazioneEst.add(new JLabel(new ImageIcon("./src/immagini/dorso90s.png")));postazioneEst.add(Box.createRigidArea(new Dimension(0,5)));}
				 postazioneEst.invalidate();
				 postazioneEst.validate();
			}
			piatto.add(Menu.rosso);piatto.add(Menu.giallo);piatto.add(Menu.blu);piatto.add(Menu.verde);
		...
		else System.out.println("mossa non valida: "+Menu.cartaScarto.toString()+ " non  compatibile con "+mano.mano.get(indiceCarta).toString());
	...
	public static void avanti(int turno,Mano manoOvest, Mano manoNord, Mano manoEst,Piatto piatto, PostazioneLaterale postazioneOvest, PostazioneGiocatore postazioneNord, PostazioneLaterale postazioneEst,Mazzo mazzo,PostazioneGiocatore postazione,Mano mano) 
	{
		...
		switch (turno)
		{
		case 1: mossaOvest(mano,manoOvest, manoNord, manoEst,  piatto, postazioneOvest,mazzo,postazione,postazioneNord); break;
		case 2: mossaNord(manoOvest, manoNord, manoEst,piatto, postazioneNord, mazzo); break;
		case 3: mossaEst(manoOvest, manoNord, manoEst,piatto, postazioneEst, mazzo); break;
		default: break;
		}
			 ...
			 if (Menu.cartaScarto.getV()==14)
			 	{ Menu.firstTime=true;
			 	  if (Menu.turno==2) 
				  {
					 ...
					 for (Carta y: manoEst.mano) 
					 {
						 postazioneNord.add(new JLabel(new ImageIcon("./src/immagini/dorso180.png")));postazioneNord.add(Box.createRigidArea(new Dimension(0,5)));}
			 	 ...
			 	 for (Carta y :manoOvest.mano) {if (y.getC()!=4) {Menu.cartaScarto.setC(y.getC());System.out.println("colore: "+y.getC());}}
		 ...
		 if (x.getV()==Menu.cartaScarto.getV()||x.getC()==Menu.cartaScarto.getC())
			{
			 ...
			 for (Carta y: manoNord.mano) {postazioneNord.add(new JLabel(new ImageIcon("./src/immagini/dorso180.png")));postazioneNord.add(Box.createRigidArea(new Dimension(0,5)));}
			 piatto.invalidate();
			 piatto.validate();
			 postazioneNord.invalidate();
			 postazioneNord.validate();
		 ...
		 if (x.getV()==Menu.cartaScarto.getV()||x.getC()==Menu.cartaScarto.getC())
			{
			 ...
			 for (Carta y: manoEst.mano) {postazioneEst.add(new JLabel(new ImageIcon("./src/immagini/dorso90s.png")));postazioneEst.add(Box.createRigidArea(new Dimension(0,5)));}
			 piatto.invalidate();
			 piatto.validate();
			 postazioneEst.invalidate();
			 postazioneEst.validate();
